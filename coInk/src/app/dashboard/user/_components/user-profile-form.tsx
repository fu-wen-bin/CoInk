...
import { User as UserType } from '@/services/users/types';
...
const userProfileSchema = z.object({
  ...
  websiteUrl: z.string().url('请输入有效的网址').or(z.string().length(0)).optional().nullable(),
...
const formFields = [
  ...
  { name: 'websiteUrl', label: '个人网站', placeholder: 'https://your.site' },
];
...
export default function UserProfileForm({ user }: UserProfileFormProps) {
  ...
  const onSubmit = async (formData: UserProfileFormData) => {
    try {
      ...
      if (formData.websiteUrl !== user.websiteUrl) {
        updateData.websiteUrl = formData.websiteUrl || null;
      }
      ...
      await updateUserMutation.mutateAsync({ userId: user.userId, ...updateData });
              ...
              {isEditing ? (
                ...
                >
                  ...
                  <Button
                    ...
                    className="px-6 py-2 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-xl hover:from-emerald-700 hover:to-green-700 disabled:opacity-50 transition-all duration-200"
                  >
                ...
                >
                  <Button
                    ...
                    className="flex items-center gap-2 px-6 py-3 rounded-xl font-medium shadow-lg bg-gradient-to-r from-emerald-600 to-green-600 text-white hover:from-emerald-700 hover:to-green-700 transition-all duration-200"
                  >
                ...
                {formFields.map((field) => (
                  <div key={field.name} className="space-y-2">
                    <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                      ...
                      {field.name === 'websiteUrl' && <Globe className="w-4 h-4" />}
                      {field.label}
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Textarea
                        ...
                        value={field.value || ''}
                        placeholder="一句话介绍自己..."
                        className="min-h-[80px] w-full rounded-xl border-2 border-gray-200 focus:border-emerald-500 focus:ring-emerald-200 transition-all duration-200 resize-none"
                      />
                    )}
                  />
                </div>
