...
function tableSortRowColumn({
  ...
  try {
    ...
    if (cellData.mergedCells.length > 0) {
      console.warn(`无法排序 ${orientation} ${index}：包含合并单元格`);
      return false;
    }
    ...
    if (dataItems.length < 2) {
      console.log('未找到可排序的数据单元格（已排除表头）');
      return false;
    }
    ...
    console.error(`排序表格 ${orientation} 时出错:`, error);
    return false;
  }
}
...
export function useTableSortRowColumn(config: UseTableSortRowColumnConfig = { direction: 'asc' }) {
  ...
  const label = useMemo(() => {
    ...
    return orientationLabels[direction] || `排序 ${selectionType?.orientation} ${direction}`;
