...
export function useLinkState(props: { editor: Editor | null; hideWhenUnavailable: boolean }) {
  ...
  const linkUi = useEditorState({
    editor,
    selector: ({ editor: ed }) => {
      if (!ed) {
        return { isVisible: false, canSet: false, isActive: false };
      }
      return {
        isVisible: shouldShowLinkButton({
          editor: ed,
          hideWhenUnavailable,
        }),
        canSet: canSetLink(ed),
        isActive: isLinkActive(ed),
      };
    },
  });

  return {
    isVisible: linkUi?.isVisible ?? false,
    canSet: linkUi?.canSet ?? false,
    isActive: linkUi?.isActive ?? false,
  };
}
