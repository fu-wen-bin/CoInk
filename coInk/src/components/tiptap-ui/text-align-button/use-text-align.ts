...
export function useTextAlign(config: UseTextAlignConfig) {
  ...
  const alignState = useEditorState({
    ...
    selector: ({ editor: ed }) => {
      if (!ed) {
        return { isVisible: false, isActive: false, canAlign: false };
      }
      return {
        isVisible: shouldShowButton({ editor: ed, align, hideWhenUnavailable }),
        isActive: isTextAlignActive(ed, align),
        canAlign: canSetTextAlign(ed, align),
      };
    },
  });

  const isVisible = alignState?.isVisible ?? false;
  const isActive = alignState?.isActive ?? false;
  const canAlign = alignState?.canAlign ?? false;
