...
export function focusNextNode(editor: Editor) {
  ...
  if (!paragraphType) {
    console.warn('Schema 中未找到 paragraph 节点类型。');
    return false;
  }
...
export function isExtensionAvailable(
  ...
  if (!found) {
    console.warn(
      `在编辑器 schema 中未找到扩展 [${names.join(', ')}]，请确认已在编辑器配置中注册。`,
    );
  }
...
export function findNodeAtPosition(editor: Editor, position: number) {
  try {
    ...
    if (!node) {
      console.warn(`在位置 ${position} 未找到节点`);
      return null;
    }
    return node;
  } catch (error) {
    console.error(`读取位置 ${position} 的节点时出错:`, error);
    return null;
  }
}
